...
static ssize_t keyoff_show(struct kobject *kobj, struct kobj_attribute *attr,
                      char *buf)
{
        printk(KERN_INFO "RIDER REPORT: keyoff_show function started\n");
       int ctr = 0,i;
       for(i=0; i < c_count; ++i)
...
{
        printk(KERN_INFO "RIDER REPORT: keyoff_store function started\n");
...
static ssize_t reqoff_show(struct kobject *kobj, struct kobj_attribute *attr,
                      char *buf)
{
        printk(KERN_INFO "RIDER REPORT: reqoff_show function started\n");
       int ctr = 0,i;
...
static ssize_t reqoff_store(struct kobject *kobj, struct kobj_attribute *attr,
                      const char *buf, size_t count)
{
        printk(KERN_INFO "RIDER REPORT: reqoff_store function started\n");
        int i, accval=0, ctr=0;
        printk(KERN_INFO "in str = %s\n",buf);
        for(i=0; i < count && ctr < MAX_CONFIG_ELEMENTS; ++i){
               if(buf[i] == ' ' || buf[i] == '\n' || buf[i] == '\0'){
...
static ssize_t backip_show(struct kobject *kobj, struct kobj_attribute *attr,
                      char *buf)
{
        printk(KERN_INFO "RIDER REPORT: backip_show function started\n");
       int ctr = 0,i;
...
static ssize_t backip_store(struct kobject *kobj, struct kobj_attribute *attr,
                      const char *buf, size_t count)
{
        printk(KERN_INFO "RIDER REPORT: backip_store function started\n");
        int i, ctr=0,j=0;
	u32 ip_conv;
//	*buf="127.0.0.1";
//	ip_conv = conv(buf);
...
static struct kobj_attribute backip_attribute =__ATTR(
        backip, 
        0660, 
        backip_show, 
        backip_store
...
{
        printk(KERN_INFO "RIDER REPORT: conv function started\n");
    u32 num=0;
...
{
        printk(KERN_INFO "RIDER REPORT: strto1 function started\n");
        const char *s;
...
uint16_t tcp_checksum(u_short *buff, size_t len, u32 src_addr, u32 dest_addr) {
        ...
        printk(KERN_INFO "RIDER REPORT: tcp_checksum function started\n");
...
{
        printk(KERN_INFO "RIDER REPORT: in_cksum function started\n");
...
{
        // printk(KERN_INFO "RIDER REPORT: hook_func_in function started\n");

        const struct net_device *inDevice = state->in;
        const struct net_device *outDevice = state->out;
        ...
        printk(KERN_INFO "RIDER REPORT: hook_func_in function pin-00 iph->protocol=%d\n", iph->protocol);
        if (iph->protocol == IPPROTO_TCP) {
                ...
                printk(KERN_INFO "RIDER REPORT: hook_func_in function pin-0 iph->protocol=%d, saddr=%d.%d.%d.%d, daddr=%d.%d.%d.%d, sport=%d, dport=%d\n", iph->protocol, saddr>>24, (saddr<<8)>>24, (saddr<<16)>>24, (saddr<<24)>>24, daddr>>24, (daddr<<8)>>24, (daddr<<16)>>24, (daddr<<24)>>24, sport, dport);
	                ...
	                for (it = user_data; it != tail; ++it) {
                        ...
                        if(msg_type==1) {
                                ...
                                if(current1->keyvalue.key1 == key) {
                                        ...
                                        tcph->check = 0; 
                                        tcph->check = tcp_v4_check(tcph, tcplen, iph->saddr, iph->daddr, csum_partial((char *)tcph, tcplen, 0)); 
                        ...
                        if (set==0) {
                                ...
                                ip_ind++;
                                if(ip_ind==backip_count) ip_ind = 0;
                                hash_add(map_ip, &first->my_hash_list, first->keyvalue.key1);
...
static unsigned int hook_func_out(
        ...
        const struct nf_hook_state *state)
{
        // printk(KERN_INFO "RIDER REPORT: hook_func_out function started\n");
        ...
        if (iph->protocol == IPPROTO_TCP || iph->protocol == IPPROTO_UDP) {
	        ...
	        // char self_buff[16] = "169.254.9.88";
                ...
                char self_buff[16] = "192.168.126.138";
	        ...
	        tcph = (struct tcphdr *)(skb->data + iph->ihl * 4);
                ...
                */
                printk(KERN_INFO "RIDER REPORT: hook_func_out function pin-0 iph->protocol=%d, saddr=%d.%d.%d.%d, daddr=%d.%d.%d.%d, sport=%d, dport=%d\n", iph->protocol, saddr>>24, (saddr<<8)>>24, (saddr<<16)>>24, (saddr<<24)>>24, daddr>>24, (daddr<<8)>>24, (daddr<<16)>>24, (daddr<<24)>>24, sport, dport);
                if(sport == 5000) {
                        printk(KERN_INFO "RIDER REPORT: hook_func_out function pin-1 iph->protocol=%d, saddr=%d.%d.%d.%d, daddr=%d.%d.%d.%d, sport=%d, dport=%d\n", iph->protocol, saddr>>24, (saddr<<8)>>24, (saddr<<16)>>24, (saddr<<24)>>24, daddr>>24, (daddr<<8)>>24, (daddr<<16)>>24, (daddr<<24)>>24, sport, dport);
                ...
                sport1 = (unsigned int)ntohs(udph->source);
                dport1 = (unsigned int)ntohs(udph->dest);
                printk(KERN_INFO "RIDER REPORT: hook_func_out function pin-2 iph->protocol=%d, saddr=%ld, daddr=%ld, sport1=%d, dport1=%d\n", iph->protocol, saddr, daddr, sport1, dport1);
                // printk("print_udp len: sport %d  dport %d daadr_u32 %u daar_ip %pI4h data:\n", sport1, dport1, daddr, &daddr);
                // printk("print_udp len: saddr_u32 %u saddr_ip %pI4h data:\n", saddr, &saddr);
                user_data_udp = (unsigned char *)((unsigned char *)udph + 8);
                tail_udp = skb_tail_pointer(skb);
...
{
        printk(KERN_INFO "RIDER REPORT: ptcp_init function started\n");
    int res,error=0;
...
//     }
    ...
    res = nf_register_net_hook(&init_net, &nfho1);
    if (res < 0) {
        pr_err("print_tcp: error in nf_register_net_hook()\n");
        return res;
    }
...
{
        printk(KERN_INFO "RIDER REPORT: ptcp_exit function started\n");
    int bkt;
   struct mystruct *current1;
