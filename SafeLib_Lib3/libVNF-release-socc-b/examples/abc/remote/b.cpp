...
void handle_ue(int conn_id, void *request, char *packet, int packet_len, int err_code) {
    ...
    // printf("RIDER REPORT: packet received.\n"); fflush(stdout);
    ...
    char *pkt = getPktBuf(conn_id);
    memcpy((void *) pkt, (void *) (packet), packet_len);
    sendData(conn_id, pkt, packet_len);
    // freeReqObj(conn_id, 1);
...
int main(int argc, char *argv[]) {
    ...
    initLibvnf(4, 128, "169.254.9.18", dataStorePorts, 131072, false);
