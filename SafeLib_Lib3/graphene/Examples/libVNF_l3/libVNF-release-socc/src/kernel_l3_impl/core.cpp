...
extern "C" int ocall_l3vnf_create_server(void *lib_netmap_desc, int len_lib_netmap_desc, char *ifname);
...
int createServer(string inter_face, string server_ip, int server_port, string protocol)
{
    ...
    lib_netmap_desc = (struct nm_desc*) malloc(sizeof(struct nm_desc));
    ocall_l3vnf_create_server(lib_netmap_desc, sizeof(struct nm_desc), if_name.c_str());
