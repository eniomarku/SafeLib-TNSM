...
int ocall_gettime(uint64_t* microsec) {
    // rider added 2020-9-23 replace default ocall_get_time
    // int retval = 0;
...
// rider added 2021-8-4 for netmap control
int ocall_l3vnf_create_server(void *lib_netmap_desc, int len_lib_netmap_desc, char *ifname) 
{
    int len_ifname = ifname ? strlen(ifname) + 1 : 0;
    size_t copied;
    
    int retval = 0;
    ms_ocall_l3vnf_create_server_t *ms;
    ...
    ms->ms_lib_netmap_desc = (lib_netmap_desc && len_lib_netmap_desc) ? sgx_copy_to_ustack(lib_netmap_desc, len_lib_netmap_desc) : NULL;
    ms->ms_ifname = (ifname && len_ifname) ? sgx_copy_to_ustack(ifname, len_ifname): NULL;

    retval = sgx_exitless_ocall(OCALL_L3VNF_CREATE_SERVER, ms);

    if (retval >= 0) {
        if (lib_netmap_desc && len_lib_netmap_desc) {
            copied = sgx_copy_to_enclave(lib_netmap_desc, len_lib_netmap_desc,
                                        READ_ONCE(ms->ms_lib_netmap_desc), len_lib_netmap_desc);
            if (!copied) {
                retval = -EPERM;
                goto out;
            }
