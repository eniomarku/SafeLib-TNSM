...
void *action(void *arg) {
    ...
    // rider added 2020-11-30 add bind to use static ports
    struct sockaddr_in my_addr;
    my_addr.sin_family = AF_INET; 
    my_addr.sin_addr.s_addr = INADDR_ANY; 
    my_addr.sin_port = htons(clientPort + my_data->id); 
    my_addr.sin_addr.s_addr = inet_addr(clientIp.c_str());
    //
    ...
    // rider added 2020-11-30 add bind to use static ports
    if (bind(socketfd, (struct sockaddr*) &my_addr, sizeof(struct sockaddr_in)) == 0) 
        printf("Binded Correctly. id=%d, port=%d\n", my_data->id, ntohs(my_addr.sin_port)); 
    else {
        printf("Unable to bind. id=%d, port=%d\n", my_data->id, ntohs(my_addr.sin_port));
	exit(-1); 
    }
    //
	...
	printf("RIDER REPORT: %d bytes received.\n", n); fflush(stdout);
        if (n <= 0) {
    ...
    // spdlog::info("lat={}, t={}", lat, t);
    if (t != 0) lat = lat / t;
    spdlog::info("Latency for {}: {} ({})", my_data->id, lat, t);
    pthread_exit(NULL);
}
